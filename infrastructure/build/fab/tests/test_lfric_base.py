...
def setup_site_specific_config_environment(tmp_path):
    """
    This sets up the environment for the mocked site_specific config class (MockSiteConfig)
    to be used by tests of LFRicBase class methods without errors. This fixture is
    automatically executed for any test in this file.
    """
    # Creates mock module with __file__ attribute
    mock_site_module = mock.MagicMock()
    mock_site_module.Config = MockSiteConfig
    mock_site_module.__file__ = str(tmp_path / "site_specific/default/config.py")
...
def test_get_directory(monkeypatch, tmpdir) -> None:
    ...
    def create_frame_info(filename):
        return (create_frame(filename), filename, None, None, None, None, None, None)
    
    # Mock inspect.stack() to return our test callers with proper frame info structure
    mock_stack = [
    ...
    with mock.patch.object(LFRicBase, 'logger', new_callable=mock.PropertyMock) as mock_logger_prop:
        ...
        assert "Could not find apps directory" in mock_logger.warning.call_args[0][0]
...
def test_preprocessor_flags(monkeypatch) -> None:
    '''Tests setting of preprocessor flags with both command line and environment variables.'''
    ...
    monkeypatch.setattr(sys, "argv", ["lfric_base.py"])  # No command line precision
    env_vars = {
        'RDEF_PRECISION': '32',
...
def test_find_source_files_step(monkeypatch) -> None:
    ...
    with mock.patch('lfric_base.FabBase.find_source_files_step', mock_super):
        ...
        lfric_base.configurator_step.assert_called_once()
        lfric_base.templaterator_step.assert_called_once_with(lfric_base.config)
...
def test_templaterator_step(monkeypatch, tmpdir) -> None:
    ...
    monkeypatch.setattr('lfric_base.input_to_output_fpath', 
                       lambda config, input_path: mock_output_path / input_path.name)
    ...
    expected_base_dir = tmp_path / "infrastructure" / "build" / "tools" / "Templaterator"
...
def test_get_rose_meta(monkeypatch) -> None:
    ...
    lfric_base = LFRicBase(name="test")
    assert lfric_base.get_rose_meta() == None
...
def test_analyse_step(monkeypatch) -> None:
    ...
    expected_ignore = ['custom_dep1', 'custom_dep2', 'netcdf', 'mpi', 'mpi_f08',
                      'yaxt', 'mod_oasis', 'xios', 'icontext', 'mod_wait']
    mock_analyse.assert_called_once_with(
        lfric_base.config,
        root_symbol=lfric_base.root_symbol,
        ignore_dependencies=expected_ignore,
